...
* System dependencies
  JRuby 1.7.6 and later (Ruby 1.9.2);
  Java 6 or later;
...
* Database creation
  ...
  $ bundle exec jruby -S rake db:setup
...
* How to run the test suite
  ...
  * Setup test environment and run all test printing report to tmp/rspec/report.html
    $ bundle exec jruby -S rake rspec:all
  * Only run tests printing report to tmp/rspec/report.html:
    $ bundle exec jruby -S rake rspec:run
...
* Deployment instructions
  ...
  * Create .war from scratch with DB recreation and assets precompilation
  	$ bundle exec jruby -S rake war:create
  * Update DB and assets (if needed) and create .war:
  	$ bundle exec jruby -S rake war:update
