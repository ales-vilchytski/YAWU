...
Lexer.prototype = {
      ...
      if (value === "0") {
        ...
        if (isOctalDigit(char_)) {
          ...
          while (index < length) {
            ...
            if (isDecimalDigit(char_)) {
              bad = true;
            } else if (!isOctalDigit(char_)) {
              break;
            }
            value += char_;
            index += 1;
    ...
    outer: while (this.peek() !== quote) {
      ...
      if (char_ === "\\") {
        ...
        case "0":
          ...
          break;
        case "u":
          char_ = String.fromCharCode(parseInt(this.input.substr(1, 4), 16));
          jump = 5;
          break;
        case "v":
          this.triggerAsync("warning", {
  ...
  nextLine: function () {
    ...
    char_ = this.scanNonBreakingSpaces();
    if (char_ >= 0) {
      this.trigger("warning", { code: "W125", line: this.line, character: char_ + 1 });
    }

    this.input = this.input.replace(/\t/g, state.tab);
    char_ = this.scanUnsafeChars();

    if (char_ >= 0) {
      this.trigger("warning", { code: "W100", line: this.line, character: char_ });
    }
